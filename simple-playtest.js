  ...
  try {
    ...
    const gameState = await page.evaluate(() => {
      return {
        ...
        maintenanceHidden: document.querySelector('#maintenance-overlay') ? 
          document.querySelector('#maintenance-overlay').classList.contains('hidden') : false
      };
    });
    ...
    if (gameState.canvasExists) {
      ...
      const debugActive = await page.evaluate(() => {
        ...
        return debugElement ? debugElement.classList.contains('visible') : false;
      });
      ...
      console.log('📸 Final screenshot saved: ./actual-game-screenshot-final.png');
    }
    ...
    console.log('=================================');
    console.log(`✅ Successfully loaded: ${gameState.gameExists ? 'YES' : 'NO'}`);
    ...
    await fs.writeFile('./actual-playtest-report.json', JSON.stringify(report, null, 2));
