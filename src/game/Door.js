...
export class Door {
  constructor(x, y, worldTransitionManager, transitionData) {
    ...
    console.log(
      `Created ${transitionData.doorType} door to ${transitionData.targetWorld} at (${x}, ${y})`
    );
  }
  ...
  draw(ctx) {
    ...
    if (this.transitionData.doorType === 'entry') {
      ...
      ctx.font = '10px Arial';
      ctx.fillText(
        this.transitionData.targetWorld.replace('-', ' ').toUpperCase(),
        0,
        -this.height / 2 - 15
      );
    } else {
  ...
  onPlayerCollision(player) {
    console.log(
      `Player entered ${this.transitionData.doorType} door to ${this.transitionData.targetWorld}`
    );
