...
class AdvancedMCPAutomation {
  constructor() {
    ...
    this.mcpContext = {
      ...
      userPreferences: {
        ...
        objectives: ['find_all_mystery_boxes', 'explore_all_areas', 'complete_challenges']
      },
      gameKnowledge: {
        knownSecrets: [],
        discoveredPaths: [],
  ...
  async startFullAutomation() {
    ...
    console.log('🔗 Integrating with external context and intelligent systems...');
    ...
    switch (source.type) {
      case 'git_commits':
        return {
          ...
          recentChanges: ['mystery box positioning', 'interaction feedback', 'visual effects'],
      ...
      case 'supabase_analytics':
        return {
          ...
          userBehavior: ['exploration patterns', 'common stuck points', 'completion rates'],
          insights: ['players often miss boxes in corners', 'debug mode usage up 40%']
        };
      ...
      case 'linear_issues':
        return {
          ...
          activeIssues: ['mystery box discovery rate', 'player guidance system'],
  ...
  adaptAutomationStrategy() {
    ...
    if (currentFocus.priorities.includes('improve discoverability')) {
      this.mcpContext.userPreferences.objectives.unshift('test_mystery_box_discoverability');
      console.log('   🎯 Added mystery box discoverability testing to objectives');
    }

    if (userBehavior.insights.includes('players often miss boxes in corners')) {
      this.mcpContext.gameKnowledge.knownSecrets.push('check_corner_areas_thoroughly');
  ...
  async initializeGameWithContext() {
    ...
    if (this.mcpContext.externalData.linear_issues.priorities.includes('improve discoverability')) {
      await this.playwright.executeAction({ type: 'keyPress', key: 'F1' });
      console.log('   🔧 Debug mode enabled based on current development focus');
    }
  ...
  async testMysteryBoxDiscoverability(decision) {
    console.log('   🕵️ Testing mystery box discoverability based on user behavior data...');

    // Use context about where users typically miss boxes
    const problematicAreas = this.mcpContext.externalData.supabase_analytics.insights;
    ...
    if (problematicAreas.includes('players often miss boxes in corners')) {
      ...
      const cornerSequence = [
        ...
        { type: 'keyPress', key: 'ArrowRight', reason: 'move_to_right_boundary' },
  ...
  async systematicMysteryBoxSearch(decision) {
    ...
    const searchPattern = this.mcpContext.gameKnowledge.knownSecrets.includes('check_corner_areas_thoroughly') ?
      'corner_priority_search' : 'standard_sweep_search';
    ...
    const gameState = await this.playwright.page.evaluate(() => {
      return window.game ? {
        mysteryBoxes: window.game.mysteryBoxes?.map(box => ({
          x: box.x, y: box.y, activated: box.activated
        })) || [],
        playerPos: { x: window.game.player?.x || 0, y: window.game.player?.y || 0 }
      } : null;
    });

    if (gameState && gameState.mysteryBoxes.length > 0) {
      console.log(`   📦 Found ${gameState.mysteryBoxes.length} mystery boxes in game state`);
      
      // Intelligent pathfinding to boxes
      for (const box of gameState.mysteryBoxes) {
        if (!box.activated) {
          await this.navigateToBoxIntelligently(box, gameState.playerPos, decision);
        }
      }
    }
  }
  ...
  async navigateToBoxIntelligently(targetBox, currentPos, decision) {
    console.log(`   🎯 Navigating intelligently to box at (${targetBox.x}, ${targetBox.y})`);

    // Use context-aware navigation
    const distance = Math.sqrt(Math.pow(targetBox.x - currentPos.x, 2) + Math.pow(targetBox.y - currentPos.y, 2));
    const strategy = distance > 200 ? 'long_distance_approach' : 'precise_positioning';
    ...
    if (Math.abs(dx) > 20) {
      ...
      for (let i = 0; i < moves; i++) {
        await this.playwright.executeAction({ type: 'keyPress', key: direction });
  ...
  async comprehensiveAreaExploration(decision) {
    ...
    const explorationSequence = [
      ...
      { area: 'center_section', movements: ['ArrowRight', 'Space', 'ArrowRight'] },
      { area: 'right_section', movements: ['ArrowRight', 'ArrowRight', 'Space'] }
    ];
    ...
    for (const section of explorationSequence) {
      ...
      for (const movement of section.movements) {
        await this.playwright.executeAction({ type: 'keyPress', key: movement });
  ...
  async intelligentChallengeCompletion(decision) {
    ...
    if (devFocus.includes('player guidance system')) {
      console.log('   🎯 Testing player guidance systems based on dev focus...');
  ...
  async performContextualAnalysis() {
    ...
    if (finalGameState) {
      const discoveredBoxes = finalGameState.mysteryBoxes.filter(box => box.activated).length;
      const totalBoxes = finalGameState.mysteryBoxes.length;
      
      console.log(`   📦 Mystery Box Discovery: ${discoveredBoxes}/${totalBoxes} (${((discoveredBoxes/totalBoxes)*100).toFixed(1)}%)`);
      
      // Context-aware insights
      const userAverage = 2.3; // From MCP context
      const discoveryTime = (Date.now() - this.automationSession.startTime) / 1000 / 60;
      ...
      const contextDecisions = this.automationSession.decisions.filter(d => d.contextUsed.length > 0);
      console.log(`   🧠 Context-driven decisions: ${contextDecisions.length}/${this.automationSession.decisions.length}`);
    }
  ...
  async generateAdvancedReport() {
    ...
    const report = {
      ...
      insights: {
        ...
        recommendationsForDevelopment: this.generateDevelopmentRecommendations()
      }
    };
    ...
    const reportPath = path.join(outputDir, `mcp-automation-${Date.now()}.json`);
  ...
  calculateContextIntegrationRate() {
    ...
    const contextDecisions = this.automationSession.decisions.filter(d => d.contextUsed.length > 0).length;
    
    return {
      rate: totalDecisions > 0 ? contextDecisions / totalDecisions : 0,
      contextDecisions,
      totalDecisions,
      rating: contextDecisions / totalDecisions > 0.8 ? 'EXCELLENT' : 
              contextDecisions / totalDecisions > 0.5 ? 'GOOD' : 'NEEDS_IMPROVEMENT'
    };
  }

  calculateIntelligentDecisionRate() {
    const intelligentActions = this.automationSession.decisions.reduce((count, decision) => {
      return count + decision.actions.filter(action => 
        action.reasoning || action.contextReasoning || action.type === 'intelligent_navigation'
      ).length;
    }, 0);

    const totalActions = this.automationSession.decisions.reduce((count, decision) => {
    ...
    return {
      ...
      intelligentActions,
      totalActions,
      rating: intelligentActions / totalActions > 0.7 ? 'HIGHLY_INTELLIGENT' :
              intelligentActions / totalActions > 0.4 ? 'MODERATELY_INTELLIGENT' : 'BASIC_AUTOMATION'
    };
  }

  calculateObjectiveCompletionRate() {
    const objectives = this.mcpContext.userPreferences.objectives;
    const completedObjectives = this.automationSession.decisions.map(d => d.objective);
    
    return {
      rate: objectives.length > 0 ? completedObjectives.length / objectives.length : 0,
      completed: completedObjectives.length,
      total: objectives.length,
      rating: completedObjectives.length === objectives.length ? 'PERFECT_COMPLETION' :
              completedObjectives.length > objectives.length * 0.7 ? 'GOOD_COMPLETION' : 'PARTIAL_COMPLETION'
    };
  }
  ...
  evaluateAutomationEffectiveness() {
    ...
    return {
      ...
      rating: overallScore > 0.8 ? 'EXCEPTIONAL_AUTOMATION' :
              overallScore > 0.6 ? 'EFFECTIVE_AUTOMATION' :
              overallScore > 0.4 ? 'BASIC_AUTOMATION' : 'NEEDS_SIGNIFICANT_IMPROVEMENT',
  ...
  identifyAutomationStrengths() {
    ...
    return strengths.length > 0 ? strengths : ['Basic automation functionality'];
  }
  ...
  generateDevelopmentRecommendations() {
    ...
    if (devContext.priorities.includes('improve discoverability')) {
      ...
      if (discoveryRate.rate < 0.8) {
        recommendations.push({
          ...
          reasoning: 'Automation struggled with discovery, matching user behavior patterns',
    ...
    if (userContext.insights.includes('debug mode usage up 40%')) {
      recommendations.push({
        priority: 'MEDIUM',
        category: 'UI/UX',
        recommendation: 'Consider making debug information more accessible or create help tooltips',
    ...
    if (intelligenceRate < 0.5) {
      recommendations.push({
        ...
        recommendation: 'Simplify navigation or add more predictable game elements',
  ...
  printExecutiveSummary(report) {
    ...
    console.log(`⏱️  Total Session Duration: ${(report.automationSession.totalDuration / 1000).toFixed(1)}s`);
    console.log(`🎯 Objectives Attempted: ${report.automationSession.decisions.length}`);
    console.log(`🔗 Context Integration Rate: ${(report.performance.contextIntegrationRate.rate * 100).toFixed(1)}% (${report.performance.contextIntegrationRate.rating})`);
    console.log(`🧠 Intelligent Decision Rate: ${(report.performance.intelligentDecisionRate.rate * 100).toFixed(1)}% (${report.performance.intelligentDecisionRate.rating})`);
    console.log(`✅ Objective Completion: ${(report.performance.objectiveCompletionRate.rate * 100).toFixed(1)}% (${report.performance.objectiveCompletionRate.rating})`);

    console.log(`\n🏆 Overall Automation Effectiveness: ${report.insights.automationEffectiveness.rating}`);
    console.log(`📊 Overall Score: ${(report.insights.automationEffectiveness.overallScore * 100).toFixed(1)}%`);

    if (report.insights.automationEffectiveness.strengths.length > 0) {
      console.log(`\n💪 STRENGTHS:`);
      report.insights.automationEffectiveness.strengths.forEach((strength, index) => {
    ...
    if (report.insights.automationEffectiveness.weaknesses.length > 0) {
      ...
      report.insights.automationEffectiveness.weaknesses.forEach((weakness, index) => {
    ...
    if (report.insights.recommendationsForDevelopment.length > 0) {
      ...
      report.insights.recommendationsForDevelopment.slice(0, 3).forEach((rec, index) => {
