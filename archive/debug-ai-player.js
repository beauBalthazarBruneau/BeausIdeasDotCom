  ...
  async startDebugSession() {
    ...
    try {
      ...
      const gameState = await this.playwright.page.evaluate(() => {
        ...
        if (window.game) {
          ...
          return {
            gameExists: false,
            canvasElement: !!document.querySelector('#game-canvas'),
            maintenanceOverlay: !!document.querySelector(
              '#maintenance-overlay'
            ),
            maintenanceHidden: document
              .querySelector('#maintenance-overlay')
      ...
      if (!gameState.gameExists) {
        ...
        console.log(
          '🔄 Retry Game State:',
          JSON.stringify(retryGameState, null, 2)
        );
      }
      ...
      if (gameState.gameExists) {
        ...
        if (afterMoveState && afterMoveState.x !== gameState.playerPosition.x) {
          ...
          console.log(
            `   Moved from x=${gameState.playerPosition.x} to x=${afterMoveState.x}`
          );
        } else {
