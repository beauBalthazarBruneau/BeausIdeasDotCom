...
async function analyzeGameplayExperience() {
  ...
  try {
    ...
    const debugToggled = await page.evaluate(() => {
      return (
        document.querySelector('#debug-info')?.classList.contains('visible') ||
        false
      );
    });
    ...
    const gameInitMessages = consoleMessages.filter(
      (msg) =>
    ...
    if (!detailedGameState.windowGame.exists) {
      analysis.recommendations.push({
        ...
        suggestion:
          'Expose game object to window for easier debugging and testing',
    ...
    // Save analysis
    await fs.writeFile(
      './detailed-gameplay-analysis.json',
    ...
    console.log(
      `💾 Memory usage: ${performanceMetrics.memoryUsage?.used || 'unknown'}MB`
    );
    console.log(
      `🎮 Input tests: ${gameplayTests.filter((t) => t.acceptable).length}/${gameplayTests.length} passed`
    );
